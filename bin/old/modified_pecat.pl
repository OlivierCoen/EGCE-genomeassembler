...
sub usage() {
    print "Usage: pecat.pl correct|first_assembly|phase|second_assembly|polish cfg_fname\n".
          "    correct:           correct rawreads\n" .
          "    first_assembly:    First assembly\n" .
          "    phase:             Phasing\n" .
          "    second_assembly:   Second assembly\n" .
          "    polish:            Polishing\n"
}
...
sub main() {
    if (scalar @ARGV >= 2) {
        ...
        } elsif ($cmd eq "first_assembly") {
            cmd_first_assembly($cfgfname);
        } elsif ($cmd eq "phase") {
            cmd_phase($cfgfname);
        } elsif ($cmd eq "second_assembly") {
            cmd_second_assembly($cfgfname);
        } elsif ($cmd eq "polish") {
            cmd_polish($cfgfname);
        } else {
            usage();
